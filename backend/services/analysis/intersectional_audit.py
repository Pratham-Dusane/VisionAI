...
# Statistical significance threshold for severity assignment.
# Below this, groups are shown but capped at LOW_CONFIDENCE.
...
def intersectional_audit(
    ...
    # Absolute floor: skip groups smaller than 3 entirely (noise)
    ...
    for col_a, col_b in combinations(protected_cols, 2):
        ...
        for val_a in df[col_a].dropna().unique():
            for val_b in df[col_b].dropna().unique():
                ...
                if di is not None:
                    if low_confidence:
                        # Below significance threshold: cap at LOW_CONFIDENCE
