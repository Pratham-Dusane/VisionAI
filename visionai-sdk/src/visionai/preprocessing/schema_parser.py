"""
Schema Parser — Auto-detect sensitive/protected columns from datasets.

Scans column names and value distributions to identify potential
protected attributes (gender, race, age, etc.) with confidence scores.
"""
...
def parse_schema(df: pd.DataFrame) -> dict:
    ...
    Returns dict with keys:
        ...
        - columns: list of column metadata dicts, each with:
            - name, dtype, unique_count, null_count, sample_values
            - sensitivity_score (0.0-1.0)
            - flagged_reason (str or None)
            - auto_flagged (bool — True if sensitivity >= 0.65)
    """
    columns = []
