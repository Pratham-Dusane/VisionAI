...
Uses Cramér's V for categorical×categorical and Eta-squared for
categorical×numeric to detect proxy variables that could enable
...
def detect_proxies(
    ...
    """
    For each non-protected column, measure statistical association
    with each protected column. Flag anything above threshold as proxy risk.

    Args:
        df: Dataset DataFrame
        protected_cols: List of protected attribute column names
        threshold: Association score threshold (default 0.3)

    Returns:
        List of proxy warning dicts sorted by association score (descending),
        each containing: proxy_column, protected_column, association_score,
        method, risk_level ('HIGH'/'MEDIUM'), explanation.
    """
    warnings = []
