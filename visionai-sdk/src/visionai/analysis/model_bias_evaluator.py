"""
Model Bias Evaluator — Counterfactual perturbation testing + Equalized Odds.

Flips protected attribute values, measures how often model predictions change.
Also computes FPR/FNR per demographic group for equalized odds analysis.
"""
...
def evaluate_model_bias(
    ...
    Returns dict keyed by protected attribute with flip_rates, max_flip_rate,
    verdict, plus '_equalized_odds' key with FPR/FNR per group.
    ...
    # Build reference columns from full dataset
    ...
    eq_odds = _equalized_odds(
        df, predict_fn, protected_cols, label_col, positive_label, feature_cols
    )
...
def _counterfactual_perturbation(df, predict_fn, protected_cols, feature_cols, n_samples):
...
def _equalized_odds(df, predict_fn, protected_cols, label_col, positive_label, feature_cols):
